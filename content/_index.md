---
title: 'Home'
type: landing

sections:

  - block: markdown
    content:
      text: |
        <div class="not-prose text-center">
          <div style="
            width:min(92vw, 900px);
            margin-left:50%;
            transform:translateX(-50%);
          ">
            <img 
               src="media/hero-cropped.jpg"
               class="w-full rounded-md"
               >
          </div>

          <h1 class="text-4xl font-bold text-gray-900 dark:text-white" style="margin-top:1.75rem; margin-bottom:0;">
            Shane Perry
          </h1>

          <p class="text-xl text-gray-700 dark:text-gray-300" style="margin-top:0.75rem; margin-bottom:0;">
            They are all living to Him - Luke 20:38
          </p>
        </div>

    design:
      spacing:
        padding: ['2rem', '0', '0rem', '0']


  - block: markdown
    content:
      text: |
        <div class="not-prose text-center">
          <h2 class="text-3xl font-bold text-gray-900 dark:text-white" style="margin-bottom:1.25rem;">
            Memories
          </h2>

          <div style="
            width:min(92vw, 850px);
            margin-left:50%;
            transform:translateX(-50%);
          ">
            <img 
               src="media/memories-preview.jpg"
               class="w-full rounded-md"
               >
          </div>

          <p class="text-lg text-gray-700 dark:text-gray-300" style="max-width:760px; margin:1.5rem auto 1.25rem auto;">
            Shared stories, photos, and videos celebrating Shane.
          </p>

          <a href=memories/>
            Read Memories
          </a>
        </div>

    design:
      spacing:
        padding: ['5rem', '0', '4rem', '0']

  - block: markdown
    content:
      text: |
        <div class="not-prose text-center">
          <h2 class="text-3xl font-bold text-gray-900 dark:text-white" style="margin-bottom:1.25rem;">
            Eleven Weeks
          </h2>

          <div style="
            width:min(92vw, 850px);
            margin-left:50%;
            transform:translateX(-50%);
          ">
            <img 
               src="media/eleven-weeks-preview.jpeg"
               class="w-full rounded-md"
               >
          </div>

          <p class="text-lg text-gray-700 dark:text-gray-300" style="max-width:760px; margin:1.5rem auto 1.25rem auto;">
            A 73-day walk through Shane’s accident, recovery, and final moments.
          </p>

          <a href=eleven-weeks/>
            Read Eleven Weeks
          </a>
        </div>

    design:
      spacing:
        padding: ['5rem', '0', '4rem', '0']

  - block: markdown
    content:
      text: |
        <div class="not-prose text-center">
          <h2 class="text-3xl font-bold text-gray-900 dark:text-white" style="margin-bottom:1.25rem;">
            About Shane
          </h2>

          <div style="
            width:min(92vw, 850px);
            margin-left:50%;
            transform:translateX(-50%);
          ">
            <img 
               src="media/about-shane-preview.JPG"
               class="w-full rounded-md"
               >
          </div>

          <p class="text-lg text-gray-700 dark:text-gray-300" style="max-width:760px; margin:1.5rem auto 1.25rem auto;">
            Photos, timeline, talks, and the ongoing visits shared in A Short Rest.
          </p>

          <a href=about-shane/>
            Explore About Shane
          </a>
        </div>

    design:
      spacing:
        padding: ['5rem', '0', '5rem', '0']
---